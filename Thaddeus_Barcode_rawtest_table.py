...
from docx.shared import Inches, lazyproperty, Parented
...
class Table(Parented):
    ...
    def change_cell(self, row_inx, col_inx, action): 
        cell_idx = col_idx + (row_idx * self._column_count)
        self._cells[cell_idx] = action
        return self._cells[cell_idx]
...
class _Cell(BlockItemContainer):
    ...
    def add_cell_picture(self, file_name='', style = None, w = 1.1, h = 0.5):

        return super(_Cell, self).add_picture(file_name, width=docx.shared.Inches(w), height=docx.shared.Inches(h))
...
barcode_doc = docx.Document()



def make_13(number):
    add_these = 12 - len(number)
    for i in range(add_these):
        number = "0" + str(number)
    return number
...
if __name__=='__main__':
    ...
    parser.add_argument('--file_name', type=str, default="barcodes for printer in table")
    parser.add_argument('--starting_number', type=int, default=0, required=False)
    ...
    for i in range(10*paper):
        number = str(starting_number)
        print(i)
        ...
        generate('EAN13', number, writer=ImageWriter(), output=number)
        starting_number += 1
        
            
    for i in range(paper):
        tables = barcode_doc.add_table(rows = 5, cols = 2)
        counter = 0
        for x in range(10):
            number = args.starting_number + i*10 + x
            number = str(number)
            if len(number) != 13:
                #print(number)
                add_these = 12 - len(number)
                for z in range(add_these):
                    number = "0" + str(number)
            if counter%2 == 0:
                cell = tables.cell(int(counter/2),0)
                cell_pic = cell.add_paragraph().add_run()
                cell_pic.add_picture(str(number)+".png", width=docx.shared.Inches(2.4), height=docx.shared.Inches(1.2))

            if counter%2 == 1: 
                cell = tables.cell(int(counter/2),1)
                cell_pic = cell.add_paragraph().add_run()
                cell_pic.add_picture(str(number)+".png", width=docx.shared.Inches(2.4), height=docx.shared.Inches(1.2))
            counter += 1
        
    barcode_doc.save(args.file_name+'.docx')
